...
export default function TechnicalResponsibilityOverview() {
  const columns: TableColumn<TechnicalResponsibility>[] = [
    { label: 'Responsável Técnico', attribute: 'responsibleName' },
  ];
  ...
  const deleteTechnicalResponsibility = async () => {
    if (!currentId) return;

    const res = await TechnicalResponsibilityService.deleteById(currentId);
    if (res.success) {
      setIsDeleteModalOpen(false);
      const itemName =
        data.find((item) => item.id === currentId)?.responsibleName || '';
      setCurrentId(null);
